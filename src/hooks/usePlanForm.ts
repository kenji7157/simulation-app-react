...
export const usePlanForm = () => {
  ...
  const [planOptions, setPlanOptions] = useState<(SelectOption & { explain: string })[]>([]);
  ...
  useEffect(() => {
    ...
    setPlanType(options.length > 0 && options[0].value ? options[0].value : planTypes.UNSELECTED);
    setExplain(options.length > 0 && options[0].explain ? options[0].explain : "");
